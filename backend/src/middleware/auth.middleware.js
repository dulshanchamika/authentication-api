...
/**
 * Middleware to populate req.user if a token exists, but doesn't block if it doesn't.
 * Useful for global security middleware that needs role info but shouldn't block public routes.
 */
export const populateUser = (req, res, next) => {
  try {
    ...
    // We don't block here, just move to next
    next();
  }
};
